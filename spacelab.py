...
def upload():
    try:
        ...
        force_mode = (request.form.get("mode") or request.args.get("mode") or "").strip().lower()

        board_analysis = None
        should_try_board = force_mode in ("board", "plateau", "multi", "auto", "") or img.shape[1] >= 1200
        if should_try_board:
            try:
                board_analysis = _analyze_full_board(img)
            except Exception as e:
                print("BOARD ANALYSIS ERROR:", e)
                board_analysis = None

        if board_analysis and board_analysis.get("ok") and board_analysis.get("cards_found", 0) > 0:
            return jsonify({
                "mode": "board",
                "board": board_analysis,
                "cards": board_analysis.get("cards", []),
                "cards_found": int(board_analysis.get("cards_found", 0)),
                "slots": board_analysis.get("slots", []),
                "capsules": board_analysis.get("capsules", []),
                "rects": [
                    {
                        "x": int(c["x"]),
                        "y": int(c["y"]),
                        ...
                        "final_card_id": c.get("final_card_id"),
                    }
                    for c in board_analysis.get("cards", [])
                ],
            })
...
def _recognize_card_in_crop(crop):
    ...
    return {
        "ok": card_match is not None,
        "reason": None if card_match is not None else "card_match_failed",
        "rect": rect,
        "warped_b64": _img_to_base64(warped),
        "signature": sig,
        "rois": rois,
...
def _analyze_full_board(img):
    ...
    for slot in analysis.get("slots", []):
        ...
        recognized = None
        if slot.get("status") == "occupied" or slot.get("score", 0.0) >= 0.40:
            recognized = _recognize_card_in_crop(crop)
            if recognized.get("final_card_id"):
                slot["status"] = "occupied"
                slot["recognized"] = True
            else:
                slot["recognized"] = False
        else:
            slot["recognized"] = False

        slot["recognition"] = recognized

        if recognized and recognized.get("final_card_id"):
            cards.append({
                ...
                "final_card_id": recognized.get("final_card_id"),
                "final_status": recognized.get("final_status"),
                "final_score": recognized.get("final_score", 0.0),
                "final_gap": recognized.get("final_gap", 0.0),
                "color_name": recognized.get("color_name"),
                "symbol_name": recognized.get("symbol_name"),
                "bottom_layout": recognized.get("bottom_layout"),
                "points": recognized.get("points"),
    ...
    analysis["cards_found"] = len(cards)
    analysis["slots_occupied"] = sum(1 for s in analysis.get("slots", []) if s.get("status") == "occupied")
    analysis["slots_recognized"] = sum(1 for s in analysis.get("slots", []) if (s.get("recognition") or {}).get("final_card_id"))
    return analysis
